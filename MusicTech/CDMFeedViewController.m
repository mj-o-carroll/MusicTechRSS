...
#import "CDMFeedViewController.h"
...
@interface CDMFeedViewController ()
{
    NSArray *_objects;
...
@implementation CDMFeedViewController
    ...
    [rss fetchRssWithURL:feedURL
                complete:^(NSString *title, NSArray *results) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        UIImage* newImage = [UIImage imageNamed:@"CDMHeader@2x.png"];
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CreativeMusicListCell" forIndexPath:indexPath];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"ToCreativeMusicArticle"]) {
        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
        RSSItem *object = _objects[indexPath.row];
        CDMArticleViewController * cmViewController = [segue destinationViewController];
        cmViewController.feedURL = object.link;
    }
}
